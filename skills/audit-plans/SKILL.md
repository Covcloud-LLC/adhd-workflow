...
1. **Format conformance** — every active plan must satisfy the `/promote` rubric: a definition of done, actionable tasks, a `Verify:` clause per task, scoped (~≤8 tasks), a one-line why, and the provider-qualified model header contract. A current plan has:
   ...
   **Brief / decision-delta rule:** a plan in the **new house format** (it carries a `Default run tier:` header) that is missing `## What this plan will actually do` or `## Decisions this plan makes` is **flagged for migration** — same severity and treatment as the legacy header flag above, never a failure. Plans still in a legacy format (no `Default run tier:` header) are **exempt from this check entirely**.
...
9. **Stale reasoning notes** — reasoning notes (e.g. `*-reasoning.md`) under the resolved docs root's `notes/` (the metarepo's `notes/` when one is configured, `./docs/notes/` in the code repo otherwise — see *Docs root* above) that no active plan references AND that are untouched > ~90 days (`git log -1 --format=%cr`). Recommend: confirm the decision still holds, or mark the note superseded. **When a metarepo is configured, also scan the code repo's own `./docs/notes/` for `*-reasoning.md` files** — those are legacy notes written before the move; they get the same staleness check plus one extra recommendation: migrate to the metarepo's `notes/`, or keep them as a deliberate exception. Match `*-reasoning.md` only — every other file in the code repo's `docs/notes/` is a durable design/decision/reference doc and is never swept. This narrows the notes-out-of-scope rule above: the audit may FLAG a note as stale, but still never treats a note as a malformed plan.
...
11. **Brief drift spot-check** — for one or two slices per audited plan, compare the brief's (`## What this plan will actually do`) sentence for that slice against the slice's own `task:` string. If the brief describes work the `task:` string doesn't contain, or vice versa, flag **"brief drift"** and recommend re-syncing the brief. Flag only, never a failure — a plan with no brief section at all is already caught by check #1.
