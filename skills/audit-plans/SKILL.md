...
1. **Format conformance** — every active plan must satisfy the `/promote` rubric: a definition of done, actionable tasks, a `Verify:` clause per task, scoped (~≤8 tasks), a one-line why, and the provider-qualified model header contract. A current plan has:
   ...
   **Legacy migration rule:** two legacy shapes are **readable, not malformed** — the previous four-line `Model:`/`OpenAI:`/`Claude:`/`Recommended:` header, and unqualified `opus`, `sonnet`, `haiku`, `fable` model names. Flag either for migration to a `Default run tier:` line plus per-slice `Run at:` lines and offer to convert; never treat them as unreadable. An `Effort` present only at plan level still drives the red-gate for every slice.
