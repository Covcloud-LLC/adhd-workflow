...
1. Resolve git root. Read every plan in `docs/plans/*.md` (skip `docs/plans/_done/`, and — by the same convention — skip any file whose basename begins with `_`: **underscore-prefixed plans are fixtures**, never counted toward WIP, never picked as `▶ NEXT`, never flagged stalled, never archived). Parse each plan's status and its per-slice statuses (the `> status:` line directly under each `### <id>` slice heading, where present). **A slice heading (`### <id> — …`) carrying a trailing ` ✅` marker is done** — bare ` ✅` (hand-confirmed via `/wrap-up`), provenance-stamped ` ✅ (<command>, <sha>)` (machine-witnessed via `/run-plan`'s two-agent gate), or ` ✅ (<command>, single-agent)` (`/run-plan`'s weaker whole-tree-only gate for exempt slices); slices without a marker are open (ignore slices explicitly delegated/moved/dropped, e.g. "→ see Plan 08"). Slices exist only as `### <id> — …` headings; a legacy bold-dialect plan (`**<ID>-n — …** · status` paragraph lines) has no findable slices — report it as needing migration (`/audit-plans` offers the conversion) instead of misreading it as sliceless. **`docs/notes/` is out of scope** — it holds design/decision/reference docs, not task-tracked plans; never read it or flag its files as drift.
...
- Relationship to a hand-maintained per-project index (e.g. a hand-written `docs/plans/00-master-index.md`): `--board` is the generated equivalent; the two may coexist, and converging them is out of scope here.
